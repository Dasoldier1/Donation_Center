	...
	<body>
		<div class="navbar navbar-fixed-top">
			...
			if ($user_role != $row['userRole'])
				{
			?>
				<center><h4><p><?php echo $lang['admin_message_dblog']; ?></p></h4></center>
					<hr />
						<blockquote>
							<?php
							class paypal_donation_log {
								...
								{
									return '<table><tr><td>Character name: ' . $this->character_name . '</td></tr><tr><td>Transaction ID: ' . $this->transaction_id . '</td></tr><tr><td>Amount: &euro; ' . $this->amount . '</td></tr><tr><td>Amount min fee: &euro; ' . $this->amountminfee . '</td></tr><tr><td>Donation reward: ' . $this->donation . '</td></tr><tr><td>Date and time: ' . $this->dt . '</td></tr></table><hr />';
								}
								
							}
							 
							try {
									$result = $database_class->dbConnection()->query('SELECT * FROM paypal_donation_log ORDER BY dt ASC');
									# Map results to object
									$result->setFetchMode(PDO::FETCH_CLASS, 'paypal_donation_log');
								
									while($donation_info = $result->fetch())
										{
											# Call our custom donation_info method
											echo ' ' . $donation_info->donation_info() . '<br>';
										}
								} 
							catch(PDOException $e) 
							{
								...
								$local_log .= "ADMIN DATABASE_LOG.PHP ERROR: ". $e->getMessage();
								$local_log .= '<hr />';
								...
								fclose($fp);  // close file
									?>
										<div class='alert alert-error'>
											...
											<center><strong><?php echo "ERROR: " . $e->getMessage(); ?></strong></center>
										</div>
									<?php
							}
							?>
